...
const EngineScreen: FunctionComponent<ViewProps> = (props: ViewProps) => {
	...
	useEffect(() => {
		if (engine) {
			...
			// envirronment
			const envUri = resolveAssetSource(
				require("./assets/environment.env")
			).uri;
			var envCube = CubeTexture.CreateFromPrefilteredData(
				envUri,
				scene,
				".env"
			);
			...
			const sceneGLBUri = resolveAssetSource(
				require("./assets/nativeStaticScene.glb")
			).uri;
			SceneLoader.AppendAsync("", sceneGLBUri, scene).then(() => {
				const starsGLBUri = resolveAssetSource(
					require("./assets/starsGeo.glb")
				).uri;
				SceneLoader.AppendAsync("", starsGLBUri, scene).then(() => {
					...
					const starsPanoramaUri = resolveAssetSource(
						require("./assets/starfield_panorama_texture.jpg")
					).uri;

					var starsShaderMaterial = new NodeMaterial(
						"starsShader",
						scene
					);
					...
					const starfieldTexture = new Texture(
						starsPanoramaUri,
						scene,
						false,
						false
					...
					if (starsShaderMaterial.getBlockByName("emissiveTex")) {
						const starfieldTextureBlock =
							starsShaderMaterial.getBlockByName("emissiveTex");
						(starfieldTextureBlock as TextureBlock).texture =
							starfieldTexture;
					}
				...
				let asteroids = [
					...
					"asteroidGroup3",
					"asteroidGroup7",
					"asteroidModule3_i148",
					"asteroidModule3_i117",
					"asteroidModule2_i107",
				];
				...
				for (let each = 0; each < asteroids.length; each++) {
					...
					if (!asteroid) {
						asteroid = scene.getTransformNodeByName(
							asteroids[each]
						);
					}
					...
					let rotationSpeed =
						Math.floor(
							Math.random() *
								(rotationSpeedMax - rotationSpeedMin + 1)
						) + rotationSpeedMin;

					const randomRotation = new Animation(
						"rotation",
						"rotation.x",
						frameRate,
					...
					for (each in scene.transformNodes) {
						...
						//randomScale *= randomScale;
						scene.transformNodes[each].scaling = new Vector3(
							randomScale,
							randomScale,
							randomScale
						);
				...
				scene.getTransformNodeByName("asteroidGroup104").scaling =
					new Vector3(1.3, 1.3, 1.3);
				scene.getTransformNodeByName(
					"asteroidGroup104"
				).parent.scaling = new Vector3(1.22, 1.22, 1.22);
				scene.getTransformNodeByName("asteroidGroup3").scaling =
					new Vector3(1, 1, 1);
				scene.getTransformNodeByName("asteroidGroup3").parent.scaling =
					new Vector3(1, 1, 1);
				scene.getTransformNodeByName("asteroidGroup105").scaling =
					new Vector3(1, 1, 1);
				scene.getTransformNodeByName(
					"asteroidGroup105"
				).parent.scaling = new Vector3(1.25, 1.25, 1.25);
				scene.getTransformNodeByName("asteroidGroup100").scaling =
					new Vector3(1, 1, 1);
				scene.getTransformNodeByName(
					"asteroidGroup100"
				).parent.scaling = new Vector3(1.25, 1.25, 1.25);
				scene.getTransformNodeByName("asteroidGroup102").scaling =
					new Vector3(1.52, 1.52, 1.52);
				scene.getTransformNodeByName(
					"asteroidGroup102"
				).parent.scaling = new Vector3(1.29, 1.29, 1.29);
				scene.getTransformNodeByName("asteroidGroup87").scaling =
					new Vector3(1.03, 1.03, 1.03);
				scene.getTransformNodeByName("asteroidGroup87").parent.scaling =
					new Vector3(1.54, 1.54, 1.54);
				scene.getTransformNodeByName("asteroidGroup84").scaling =
					new Vector3(1.28, 1.28, 1.28);
				scene.getTransformNodeByName("asteroidGroup84").parent.scaling =
					new Vector3(1.47, 1.47, 1.47);
				scene.getMeshByName("asteroidModule2_i30").parent.scaling =
					new Vector3(1.13, 1.13, 1.13);
				scene.getMeshByName("asteroidModule1_i13").parent.scaling =
					new Vector3(1, 1, 1);
				...
				const sunUri = resolveAssetSource(
					require("./assets/sun.png")
				).uri;
				const planetUri = resolveAssetSource(
					require("./assets/planetImage.png")
				).uri;

				// volumetric sun
				var sun = new VolumetricLightScatteringPostProcess(
					"volumetric",
					1.0,
					camera,
					undefined,
				...
				sun.mesh.material.diffuseTexture = new Texture(
					sunUri,
					scene,
					true,
					false,
				...
				// planet
				var planet = MeshBuilder.CreatePlane(
					"planetPlane",
					{ size: 100 },
					scene
				);
				planet.position.x = 200;
				planet.billboardMode =
					Mesh.BILLBOARDMODE_ALL | Mesh.BILLBOARDMODE_USE_POSITION;

				var planetMaterial = new StandardMaterial("", scene);
				planetMaterial.backFaceCulling = false;
				planetMaterial.disableLighting = true;

				planetMaterial.diffuseTexture = new Texture(
					planetUri,
					scene,
					true,
					false,
			...
			if (xrSession) {
				...
				if (rootNode !== undefined && scene !== undefined) {
					...
					if (msh) {
						...
						msh.rotationQuaternion = Quaternion.FromEulerAngles(
							0,
							Math.PI,
							0
						);
